  ...
  return (
    <div className="flex flex-col md:flex-row items-start justify-center gap-8 h-full relative">
      <div className="w-full md:w-1/2 pt-16">
        <h3 className="text-3xl font-bold mb-4 text-navy-900">{title}</h3>
        {Array.isArray(description) ? (
          <ul className="list-disc pl-5 mb-4 space-y-2">
            {description.map((d, idx) => (
              <li key={idx} className="text-base text-navy-600">{d}</li>
            ))}
          </ul>
        ) : (
          <p className="text-base text-navy-600 mb-2">{description}</p>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 rounded-4xl p-2 shadow-md shadow-black/5">
          <div className="rounded-3xl bg-white p-10 pb-9 ring-1 shadow-2xl ring-black/5">
            ...
            <div className="flex flex-col items-center w-full">
              <UserIcon className="h-10 w-10 text-navy-700 mb-4" />
              <h3 className="text-lg font-semibold text-navy-800 mb-2 text-center">
              ...
              <div className="flex justify-between items-center w-full">
                <span className="text-navy-800 font-medium">Available</span>
                <Switch
                  checked={isAvailable}
                  onChange={setIsAvailable}
                  className={`${isAvailable ? 'bg-green-600' : 'bg-navy-200'} relative inline-flex h-6 w-11 items-center rounded-full`}
                >
