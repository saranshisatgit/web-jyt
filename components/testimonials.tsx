...
function TestimonialCard({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 rounded-3xl bg-linear-to-t from-black from-[calc(7/16*100%)] ring-1 ring-navy-950/10 ring-inset sm:from-25%"
      />
...
function CallToAction({ text, linkText, linkUrl }: CallToActionProps) {
  return (
    <div>
      <p className="max-w-sm text-sm/6 text-navy-600">{text}</p>
      <div className="mt-2">
        <Link
          href={linkUrl}
            ...
            {Array.from({ length: totalScrollPages }).map((_, pageIndex) => {
              ...
              return (
                <Headless.Button
                  ...
                  className={clsx(
                    'size-2.5 rounded-full border border-transparent bg-navy-300 transition',
                    'data-active:bg-navy-400 data-hover:bg-navy-400',
